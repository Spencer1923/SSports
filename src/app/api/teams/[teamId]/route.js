export async function GET(request, { params }) {
  ...
  try {
    ...
    const [teamRes, scheduleRes, rosterRes, depthRes] = await Promise.all([
      fetch(
    ...
    if (!teamRes.ok || !scheduleRes.ok || !rosterRes.ok || !depthRes.ok) {
      return Response.json(
        { error: "One or more ESPN requests failed" },
        { status: 502 },
      );
    }
    ...
    return Response.json({ team: team.team, schedule, roster, depth });
  } catch (err) {
